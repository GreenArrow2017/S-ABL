...
class SelfAttn(nn.Module):
    def __init__(self, hidden_size, num_attention_heads, dropout = 0.5):
        ...
        self.dropout = dropout
        self.num_attention_head_size = int(hidden_size / num_attention_heads)
        self.all_head_size = int(self.num_attention_heads * self.num_attention_head_size)
...
class NumberEncoder(nn.Module):
    def __init__(self, number_vocab_size, embedding_size, hidden_size, num_layers, dropout = 0.5):
        super(NumberEncoder, self).__init__()
        self.hidden_size = hidden_size
        self.embedding = nn.Embedding(number_vocab_size, embedding_size, padding_idx=0)
        self.bigru = nn.GRU(embedding_size, hidden_size, num_layers, dropout = dropout, bidirectional = True)
        self.attn = SelfAttn(hidden_size, num_attention_heads=8)
    def forward(self, x):
        # x是一个list，一共batch_size个，每一个元素是一个number list
        # (2/5) : [12, 3, 13, 6, 14]
        batch_size = len(x)
        nums_hidden = []
        length = 0
        for b in range(batch_size):
            number_list =  x[b]
            number_list_rep = []
            length = max(length, len(number_list))
            for number in number_list:
                nums_char_embedded = self.embedding(torch.LongTensor(number).cuda()).unsqueeze(1)
                nums_gru_out, nums_gru_hidden = self.bigru(nums_char_embedded)
                nums_gru_final = nums_gru_out[-1, :, :self.hidden_size] + nums_gru_out[0, :, self.hidden_size:]
                number_list_rep.append(nums_gru_final.squeeze(0))
            nums_hidden.append(number_list_rep)
        mask = []
        for b in range(batch_size):
            num_list_rep = nums_hidden[b]
            num_mask = [1 for i in num_list_rep]
            while len(num_list_rep) < length:
                num_mask.append(0)
                num_list_rep.append(self.embedding(torch.LongTensor([0]).squeeze(0).cuda()))
            nums_hidden[b] = torch.stack(nums_hidden[b], 0)
            mask.append(torch.tensor(num_mask))
        nums_hidden = torch.stack(nums_hidden, 0)
...
if __name__ == '__main__':
    enc = NumberEncoder(12, 128, 128, 2)
    number_lists = [['1', '1.5', '10%', '12'], ['23', '4.56', '23%']]
    enc(number_lists)
