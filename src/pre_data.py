...
class Lang:
    ...
    def trim(self, min_count):  # trim words below a certain count threshold
        ...
        print('keep_words %s / %s = %.4f' % (len(keep_words), len(self.index2word), len(keep_words) / len(self.index2word)))
...
def prepare_data(pairs_trained, pairs_tested, trim_min_count):
    ...
    for pair in pairs_trained:
        ...
        train_pairs.append((input_cell, len(input_cell), pair[1], pair[2], pair[3], pair[4]))
    print('Indexed %d words in input language, %d words in output' % (input_lang.n_words, output_lang.n_words))
    print('Number of training data %d' % (len(train_pairs)))
    for pair in pairs_tested:
        input_cell = indexes_from_sentence(input_lang, pair[0])
        test_pairs.append((input_cell, len(input_cell), pair[1], pair[2], pair[3], pair[4]))
...
def prepare_de_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    for pair in pairs_tested:
        ...
        test_pairs.append((input_cell, len(input_cell), output_cell, len(output_cell), pair[2], pair[3], num_stack))
    ...
    while idx < len(ex):
        ...
        if (s == "*" or s == "+") and random.random() < rate:
            ...
            if s == "+":
                ...
                flag = 0
                while not (lidx == -1 or ((ex[lidx] == "+" or ex[lidx] == "-" or ex[lidx] == "*" or ex[lidx] == "/") and flag == 0) or flag == 1):
                    if ex[lidx] == ")" or ex[lidx] == "]":
                        flag -= 1
                    elif ex[lidx] == "(" or ex[lidx] == "[":
                        flag += 1
                ...
                flag = 0
                while not (ridx == len(ex) or ((ex[ridx] == "+" or ex[ridx] == "-" or ex[ridx] == "*" or ex[ridx] == "/") and flag == 0) or flag == -1):
                    if ex[ridx] == ")" or ex[ridx] == "]":
                        flag -= 1
                    elif ex[ridx] == "(" or ex[ridx] == "[":
                        flag += 1
