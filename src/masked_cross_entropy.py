...
def sequence_mask(sequence_length, max_len=None, device = 'cuda:1'):
    if max_len is None:
...
def masked_cross_entropy_usage(logits, target, length, weights):
    ...
    loss = (weights.sum(1).mul(losses.sum(1))).sum() / length.float().sum()
    # if loss.item() > 10:
    #     print(losses, target)
    return loss

def masked_cross_entropy(logits, target, length, device = 'cuda:0'):
